...
Allows the handler to pass any data down to the React route. The data will be
available within the `data` prop of the relative route.

The `Response::Props` variant take a `Props` struct as first argument that
represents the data.
...
Allows the server to redirect the current request to any path. For example:
...
This variant allows the route handler to skip the ReactJS server-side rendering
and return the values passed.

For example to generate the app's sitemaps and return an `.xml` file we could
