...
pub fn create_new_project(folder_name: Option<String>, template: Option<String>) {
    ...
    let res_tag = client
        ...
        .send()
        .expect(&format!("Failed to call the tag github API for v{cli_version}"))
    ...
    let res_tree = client
        .get(&format!("{}{}?recursive=1", GITHUB_TUONO_TAG_COMMIT_TREES_URL, sha_tagged_commit))
        .send()
        .expect(&format!("Failed to call the tagged commit tree github API for v{cli_version}"))
